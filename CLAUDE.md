...
1. 노션에서 태스크 읽어오기
2. GitHub 이슈 생성 (노션 태스크 링크 본문에 첨부)
3. 이슈 번호로 브랜치 생성: `feature/#{이슈번호}-{태스크제목}` (예: `feature/#123-create-user`)
   - 기본적으로 API 하나당 이슈/브랜치 하나
   - 기능 하나에 API가 여러 개면 브랜치를 이어서 생성 (`feature/#1` → `feature/#2` …)
4. 브랜치에서 개발
   - 테스트 코드 작성 필수
   - Swagger 문서 작성 필수
5. PR 생성 (본문에 `Closes #N` 포함, 최소 1인 코드 리뷰 및 승인 필수)
6. merge 후 CI/CD 자동 배포
