...
//    }
    ...
    public ContainerResult provision(ContainerProvisionCommand command, Long requesterId) {
        ...
        // 2. 트랜잭션 커밋 후 비동기로 Docker 생성 처리하도록 이벤트 발행
        eventPublisher.publishEvent(
                new ContainerCreatingEvent(id, physicalServerId, command.cpu(), command.memory())
        );

        // 3. 사용자에게는 즉시 CREATING 상태로 응답
        return ContainerResult.from(pending);
    }
