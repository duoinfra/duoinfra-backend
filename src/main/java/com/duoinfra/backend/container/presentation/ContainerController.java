...
public class ContainerController {
    ...
    public ResponseEntity<ContainerDetailResponse> provision(@Valid @RequestBody ContainerProvisionRequest request,
        ...
        return ResponseEntity.ok(ContainerDetailResponse.from(result));
    }

    @Operation(summary = "서버 목록 조회", description = "USER는 본인 소유 서버만, ADMIN은 전체 사용자의 서버를 조회합니다.")
    @GetMapping
    ...
    @Operation(summary = "서버 상세 조회", description = "본인 소유 서버가 아니면 ADMIN이 아닌 한 404를 반환합니다.")
    @GetMapping("/{id}")
    ...
    @Operation(summary = "서버 메트릭 조회", description = "본인 소유 서버가 아니면 ADMIN이 아닌 한 404를 반환합니다.")
